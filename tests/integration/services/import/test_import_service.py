...
async def test_import_service(
    ...
    assert sherlock.user_channel == "Hulu"
    assert sherlock.user_notes == "Fun"
    ...
    assert battlestar.user_channel == "DVD"
    assert battlestar.user_notes == "Better than the 1970s version"
...
async def test_import_service_raises_on_malformed_json(
    ...
    with pytest.raises(InvalidImportDataError) as excinfo:
        await sut.import_(data)
    ex = excinfo.value
    assert isinstance(ex.__cause__, ValidationError)
    assert ex.message == "Import file validation failed"
    assert "key must be a string at line 1 column 4" in ex.details
...
async def test_import_service_raises_on_schema_invalid_data(
    ...
    assert isinstance(ex.__cause__, ValidationError)
    assert ex.message == "Import file validation failed"
    assert "Extra inputs are not permitted" in ex.details
